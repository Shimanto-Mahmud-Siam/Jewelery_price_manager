...
class ProductTemplate(models.Model):
    ...
    def _compute_gold_silver_base_price(self):
        ...
        for product in self:
            ...
            if price_field_name:
                base_price = getattr(latest_prices, price_field_name, 0.0) or 0.0
    ...
    def _compute_final_sale_price(self):
        for product in self:
            if product.product_karat:
                # Guard against negative inputs by treating negatives as zero
                weight = max(0.0, product.weight_in_grams or 0.0)
                labor = max(0.0, product.labor_cost or 0.0)
                base_price_total = product.gold_silver_base_price * weight
                product_price_with_labor = base_price_total + labor
    ...
    def _compute_weight_in_bhori(self):
        for product in self:
            # Compute regardless of sign but use absolute to avoid math issues; UI sanitizes negatives
            weight_val = product.weight_in_grams or 0.0
            if weight_val != 0.0:
                total_bhori = weight_val / config.GRAMS_PER_BHORI
    ...
    @api.onchange('weight_in_grams', 'labor_cost')
    def _onchange_clamp_non_negative(self):
        """Clamp negative form inputs to 0.0 to allow immediate saves without blocking errors."""
        for product in self:
            if product.weight_in_grams is not None and product.weight_in_grams < 0:
                _logger.warning('Clamping negative weight_in_grams to 0.0 for product %s', product.id)
                product.weight_in_grams = 0.0
            if product.labor_cost is not None and product.labor_cost < 0:
                _logger.warning('Clamping negative labor_cost to 0.0 for product %s', product.id)
                product.labor_cost = 0.0

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if 'weight_in_grams' in vals and vals.get('weight_in_grams') is not None and vals.get('weight_in_grams') < 0:
                _logger.warning('Clamping negative weight_in_grams to 0.0 on create')
                vals['weight_in_grams'] = 0.0
            if 'labor_cost' in vals and vals.get('labor_cost') is not None and vals.get('labor_cost') < 0:
                _logger.warning('Clamping negative labor_cost to 0.0 on create')
                vals['labor_cost'] = 0.0
        return super().create(vals_list)

    def write(self, vals):
        mutable_vals = dict(vals)
        if 'weight_in_grams' in mutable_vals and mutable_vals.get('weight_in_grams') is not None and mutable_vals.get('weight_in_grams') < 0:
            _logger.warning('Clamping negative weight_in_grams to 0.0 on write')
            mutable_vals['weight_in_grams'] = 0.0
        if 'labor_cost' in mutable_vals and mutable_vals.get('labor_cost') is not None and mutable_vals.get('labor_cost') < 0:
            _logger.warning('Clamping negative labor_cost to 0.0 on write')
            mutable_vals['labor_cost'] = 0.0
        return super().write(mutable_vals)
